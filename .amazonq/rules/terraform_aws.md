...
- Use IAM roles with the principle of least privilege
  - Use wildcards in actions for a given service (for instance, "s3:*" for S3 permissions) but not for resources (for instance, "arn:aws:s3:::my-bucket/*" instead of "*") to simplify the handling of permissions while keeping a good security level
  - Use IAM conditions to restrict permissions when possible (for instance, restrict S3 permissions to a specific VPC or a specific IP range)
...
- Avoid hardcoding region names or account IDs in code, use AWS datasource instead to retrieve them dynamically
